...
build:
	...
	go build -o dtx-edge -ldflags="\
    	-X 'github.com/Dtx-validator/dtx-node/versioning.Version=$(LATEST_VERSION)' \
		-X 'github.com/Dtx-validator/dtx-node/versioning.Commit=$(COMMIT_HASH)'\
		-X 'github.com/Dtx-validator/dtx-node/versioning.Branch=$(BRANCH)'\
		-X 'github.com/Dtx-validator/dtx-node/versioning.BuildTime=$(TIME)'" \
	main.go
...
test-e2e:
	...
	go build -race -o artifacts/dtx-edge .
	env EDGE_BINARY=${PWD}/artifacts/dtx-edge go test -v -timeout=30m ./e2e/...
